...
if($row = $result->fetch_assoc()) {
			...
			$date = date('Y-m-d');
			$baptism_doc_location = $row[baptism_doc_location];
			$gender = $row[gender];
...
?>		
		...
		<div class="row" style="border: 1px solid black; margin: 0px;">
			...
			<div class="col-md-12">
				...
				<?php if($baptism_doc_location!='') {?>
				<label>Upload baptism certificate: </label> 
				<a href="http://syromalabarsouthjersey.com/portal/<?php echo $baptism_doc_location;?>">View</a>
				<?php } ?> 
				<br />
			</div>
		</div>
...
<?php } ?>
		...
		<div class="row">
			<a href="students.php">
			<button type="button"   name="form_admission_submit"  
			class="btn btn-<?php echo $configure_theme;?>">Close</button>
			</a>
